...
- **Premarket** (`premarket.yml`): weekdays **`15 13 * * 1-5` UTC** → **~9:15 AM Eastern in EDT** (~8:15 AM Eastern in EST); finishes before **9:30** cash open when Actions starts on time. Uploads `premarket-context` (must include `snapshots` in `premarket_context.json`).
- **Live Trading Loop** (`live_loop.yml`): weekdays **`28 13 * * 1-5` UTC** → **~9:28 AM Eastern in EDT** (~8:28 AM Eastern in EST). Scheduled runs skip if another Live workflow already **succeeded today** (America/New_York). Runtime window defaults **09:30–12:00 ET** (`TRADING_WINDOW_*`). Artifacts: `llm-advisor-daily-news-*`.
