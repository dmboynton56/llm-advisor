...
- **Premarket** (`premarket.yml`): **dual weekday UTC crons** `20 13` / `20 14` → ~**9:20 AM America/New_York** in EDT vs EST; **`schedule` events** must land in **`scripts/check_market_open.py --scheduled-et-window`** (~09:10–10:05 ET) or remaining steps no-op (success). Cron fields are **`MINUTE HOUR`** (`13 15` is ~11:13 EDT — wrong). `workflow_dispatch` skips that gate.
- **Live Trading Loop** (`live_loop.yml`): dual **`26 13` / `26 14` UTC** → ~**9:26** NY wall-clock target + **`schedule`** ET guard (**~09:18–10:12**); still skips if another Live already **succeeded today** (ET date).
